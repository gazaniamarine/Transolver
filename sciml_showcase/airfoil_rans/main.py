import sys
import os
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "Airfoil-Design-AirfRANS")))
...
n = max(1, int(.1 * len(manifest_train)))
train_dataset = manifest_train[:-n]
val_dataset = manifest_train[-n:]
print("start load data")
train_dataset, coef_norm = Dataset(train_dataset, norm=True, sample=args.sample, n_boot=args.n_boot, my_path=args.my_path)
val_dataset = Dataset(val_dataset, sample=args.sample, n_boot=args.n_boot, coef_norm=coef_norm, my_path=args.my_path)
...
params_path = osp.join(osp.dirname(__file__), 'params.yaml')
if not osp.exists(params_path):
    params_path = osp.abspath(osp.join(osp.dirname(__file__), '..', '..', 'Airfoil-Design-AirfRANS', 'params.yaml'))
with open(params_path, 'r') as f:  # hyperparameters of the model
...
if bool(args.score):
    ...
    coefs = metrics.Results_test(device, [models], [hparams], coef_norm, args.my_path, path_out='scores', n_test=min(3, len(test_dataset)),
                                 criterion='MSE', s=s)
