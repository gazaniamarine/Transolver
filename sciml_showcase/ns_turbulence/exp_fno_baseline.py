"""
Fourier Neural Operator (FNO) Baseline for 2D NS Turbulence
============================================================
...
parser.add_argument('--n-hidden', type=int, default=64, help='FNO hidden channels')
parser.add_argument('--n-layers', type=int, default=4, help='Number of spectral conv layers')
parser.add_argument('--modes', type=int, default=12, help='Fourier modes count')
...
parser.add_argument('--save_name', type=str, default='ns_turbulence_fno')
...
def save_vorticity_comparison(pred, gt, error, idx, epoch, save_dir, vmin=-3, vmax=3):
    ...
    ax1.set_title('FNO Prediction', fontsize=13, fontweight='bold')
    ax1.axis('off')
    ...
    fig.suptitle(f'2D NS Turbulence — FNO Sample {idx}  |  Epoch {epoch}',
                 fontsize=14, y=1.02)
...
def save_training_curve(train_losses, test_losses, save_dir):
    ...
    ax.set_title('FNO Baseline Training on 2D Turbulent Navier-Stokes', fontsize=14,
                 fontweight='bold')
...
def load_data():
    ...
    # For FNO, inputs must be (N, C, H, W) where C=1
...
def main():
    ...
    print(f"\n[Model] Building FNO Model with hidden_channels={args.n_hidden}, modes=({args.modes}, {args.modes}) ...")
    model = FNO(
    ...
    for ep in range(1, args.epochs + 1):
        ...
        for fx, y in train_loader:
            fx, y = fx.cuda(), y.cuda()
            bsz = fx.shape[0]
            pred = model(fx)               # (B, 1, H, W)
    ...
    # Final summary
    save_training_curve(train_history, test_history, SAVE_DIR_RESULTS)
    print(f"\n[Done] Best FNO test Rel-L2 = {best_test_l2:.5f}")
...
def evaluate(model, loader, loss_fn, H, y_mean, y_std, epoch,
    ...
    """Run test evaluation and save vorticity comparison plots."""
    model.eval()
    total_l2  = 0.0
    n_total   = 0
    shown     = 0
    ...
    print(f"  [Eval FNO epoch={epoch}] Test Rel-L2 = {avg_l2:.5f}  (N={n_total})")
